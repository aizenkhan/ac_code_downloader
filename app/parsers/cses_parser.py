...
from app.utils.constants import (
    CSES_AC_PROBLEM_CLASS,
    CSES_ACCOUNT_ANCHOR_CLASS,
    CSES_AC_PROBLEM_TITLE_CLASS,
    CSES_BASE_URL,
    CSES_CODE_LINK_CONTENT_CLASS,
    CSES_CSRF_TOKEN_FORM_NAME,
    CSES_LOGIN_URL,
    CSES_NEXT_PAGE_CLASS,
    CSES_PAGE_SPAN_CLASS,
    CSES_PASSWORD_FORM_NAME,
    CSES_PASSWORD_YAML_NAME,
    CSES_PREV_PAGE_CLASS,
    CSES_PROBLEM_RESULTS_URL,
    CSES_PROBLEMS_URL,
    CSES_TABLE_CLASS,
    CSES_TABLE_CODE_LANG_COL,
...
_cses_config_abs_path = get_abs_path(
    rel_path=CSES_YAML_REL_PATH, caller_script_directory=__file__
)

_cses_out_abs_dir = get_abs_path(
    rel_path=CSES_REL_OUT_DIR, caller_script_directory=__file__
)
...
class CsesParser(BaseParser):
    ...
    def preprocess(self, session):
        ...
        validate_keys(
            self.local_context,
            [CSES_USERNAME_YAML_NAME, CSES_PASSWORD_YAML_NAME],
            to_prompt=True,
        )

        # log in
        response = http_get(session, CSES_LOGIN_URL, self.headers, True)

        # extract csrf token
        content = parse(response.text)
        csrfToken = content.find_all(
            "input", attrs={"name": CSES_CSRF_TOKEN_FORM_NAME}
        )[0]["value"]
        ...
        # post
        response = http_post(session, CSES_LOGIN_URL, payload, self.headers)
        ...
        self.user_id = post_login_content.find_all(
            "a", attrs={"class": CSES_ACCOUNT_ANCHOR_CLASS}
        )[0]["href"]
    ...
    def process(self, session):
        ...
        list_ac_problem_links = [
            CSES_PROBLEM_RESULTS_URL + "/" + link["href"].strip("/").split("/")[-1]
            for link in _list_ac_problem_tags
        ]
        ...
        for link in list_ac_problem_links:
            ...
            _problem_name = (
                ac_problem_content.find_all(
                    "div", attrs={"class": CSES_AC_PROBLEM_TITLE_CLASS}
                )[0]
                .find("h1")
                .text
            )
            ...
            _list_page_anchor_tags = ac_problem_content.find_all(
                "div", attrs={"class": CSES_PAGE_SPAN_CLASS}
            )[0].findAll(
                "a",
                class_=lambda val: val
                not in [CSES_NEXT_PAGE_CLASS, CSES_PREV_PAGE_CLASS],
            )
            list_page_links = [
                CSES_BASE_URL + anchor_tag["href"]
            ...
            for page_link in list_page_links:
                ...
                for row in _table_element.findAll("tr")[1:]:
                    ...
                    if (
                        _curr_dict.get(CSES_TABLE_VERDICT_COL, {}).get("class")
                        != CSES_AC_PROBLEM_CLASS
                    ):
                        continue
                    ...
                    for key in _curr_dict:
                        if key not in [
                            ...
                            _curr_dict[key] = _curr_dict[key].text.lower()

                    # extract code page link from submission column
                    _curr_dict[CSES_TABLE_SUBMISSION_COL] = CSES_BASE_URL + _curr_dict[
                        CSES_TABLE_SUBMISSION_COL
                    ].find("a").get("href")
            ...
            df.drop(
                [CSES_TABLE_TIMESTAMP_COL, CSES_TABLE_VERDICT_COL], inplace=True, axis=1
            )

            # 2. remove 's' and 'ch' from 'code time' and 'code size' columns resp. and make them numeric
            df[[CSES_TABLE_CODE_TIME_COL, CSES_TABLE_CODE_SIZE_COL]] = (
                df[[CSES_TABLE_CODE_TIME_COL, CSES_TABLE_CODE_SIZE_COL]]
            ...
            df = df.loc[
                df.groupby(CSES_TABLE_CODE_LANG_COL)[CSES_TABLE_CODE_TIME_COL].idxmin()
            ...
            for index in range(df.shape[0]):
                ...
                code_link_content = parse(response.text)

                # get submitted code
                code = code_link_content.find_all(
                    "pre", attrs={"class": CSES_CODE_LINK_CONTENT_CLASS}
                )[0].text
                ...
                path_to_write = join(
                    ...
                    _problem_name,
                    code_lang,
                    f"{compressed_problem_name}.{EXTENSION_MAPPING.get(code_lang)}",
                )
                write_all_data_to_file(path_to_write, code, create_parent_dirs=True)
